(() => {
  return {
    onLoad: () => {
      try {
        const revenge = window.revenge || window.vendetta || globalThis.revenge || globalThis.vendetta;
        const commands = revenge?.commands;

        if (!commands) {
          console.error("[TimestampCommand] Revenge commands API not found.");
          return;
        }

        this.unregister = commands.registerCommand({
          name: "t",
          displayName: "t",
          ...
          options: [
            ...
            { name: "year", displayName: "year", description: "Year (e.g. 2026)", displayDescription: "Year (e.g. 2026)", type: 4, required: true },
          ...
          execute: (args) => {
            try {
              ...
              if (isNaN(unix)) return { content: "Invalid date provided!" };
